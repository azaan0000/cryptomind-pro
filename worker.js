...
function generateSignalW(ind, symbol, candles, htfData, deriv){
  ...
  // Hard gate 1: HTF alignment (same rule as the app — 2+ of 1h/4h/1d against => veto)
...
async function runAutoTradeScan(env){
  ...
  if(!config.userId || !config.realAutoTrade || !config.riskAccepted){
    await appendBotLog(env,{ skipped:'bot off or risk not accepted' });
    return;
  }

  let creds;
  try{ creds = await getCreds(env, config.userId, 'binance'); }
  catch(e){ await appendBotLog(env,{ skipped:'exchange not connected' }); return; }
  ...
  try{
    ...
    await appendBotLog(env,{ error:'could not read open positions (relay offline?): '+e.message });
    return; // safer to skip this cycle than trade blind without knowing current exposure
  }
  if(openCount>=3){ await appendBotLog(env,{ skipped:'max 3 concurrent real positions reached' }); return; }
  ...
  if(available!=null){
    ...
    if(margin<1){ await appendBotLog(env,{ skipped:'available balance too low to size a trade' }); return; }
  }

  const candidates = [];
  const scanList = WATCHLIST.filter(c=>!openSymbols.has(c.sym));

  await Promise.all(scanList.map(async (coin)=>{
    try{
      ...
      if(sig.direction!=='HOLD' && sig.confidence>=70){
  ...
  const chosen = candidates.slice(0, Math.max(0,slotsLeft));

  if(chosen.length===0){
    await appendBotLog(env,{ scanned:scanList.length, qualifying:candidates.length, opened:0 });
    return;
  }
